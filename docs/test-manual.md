...
| 1-5 | アクションボタンを確認 | Separator下に「FFmpegの導入」「実行方法」「ライブラリの追加」の3ボタンが中央寄せで表示 | |
...
| 9-5 | モバイル固定バー | 画面下部に「FFmpegの導入」「実行方法」「ライブラリの追加」3ボタンが常時表示 | |
| 9-6 | デスクトップでは固定バー非表示 | sm幅以上では画面下部固定バーが非表示 | |
| 9-7 | モバイルでコンテンツが固定バーに隠れない | 本体コンテンツに `pb-20` が適用され、下部余白が確保されている | |
