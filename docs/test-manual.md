...
|---|-----------|---------------------|-------|
| 3-1 | 画像圧縮（AVIF） | `ffmpeg -y -i input.png ... output.avif` | |
| 3-2 | 画像圧縮（WebP） | `ffmpeg -y -i input.png ... output.webp` | |
| 3-3 | 動画圧縮（AV1） | `ffmpeg -y -i input.mp4 -c:v libsvtav1 ... output.mp4` | |
| 3-4 | 動画フォーマット変換 | `ffmpeg -y -i input.mp4 ... output.webm` | |
| 3-5 | 音声抽出 | `ffmpeg -y -i input.mp4 -vn ... output.mp3` | |
| 3-6 | 音声変換 | `ffmpeg -y -i input.wav ... output.mp3` | |
| 3-7 | 動画トリム | `ffmpeg -y -ss ... -i input.mp4 ... output.mp4` | |
| 3-8 | GIF生成 | パレット生成 + GIF生成の2コマンド | |
...
| 3-12 | スライダー（CRF等）を変更 | コマンドの数値が即座に反映 | |
...
| # | 手順 | 期待結果 | OK/NG |
|---|------|---------|-------|
| 4-1 | ファイルをD&Dエリアにドラッグ | ドロップゾーンがハイライト表示 | |
| 4-2 | 単一ファイルをドロップ | ファイル名が検出され、コマンドの入力ファイル名に反映 | |
| 4-3 | 複数ファイルをドロップ | バッチモードに切り替わり、ファイル一覧が表示 | |
| 4-4 | バッチモード時のタブ切替 | Bash / PowerShell / cmd タブが切替可能 | |
| 4-5 | クリアボタン | ファイルがクリアされ、通常モードに戻る | |
...
| 6-4 | Linuxタブをクリック | Ubuntu/Debian, Fedora, Arch Linux の3ディストリビューションが表示 | |
| 6-5 | インストール確認セクション | `ffmpeg -version` コマンドが表示 | |
| 6-6 | 公式サイトリンク | ffmpeg.org へのリンクが表示され、クリックで別タブで開く | |
| 6-7 | Windowsの gyan.dev リンク | 別タブで開く | |
| 6-8 | ×ボタンでモーダルを閉じる | モーダルが閉じる | |
...
| 8-5 | インストールガイドモーダルを開く | 英語で表示される | |
| 8-6 | 実行方法モーダルを開く | 英語で表示される | |
| 8-7 | 日本語に戻す | UI全体が日本語に戻る | |
| 8-8 | リロード | 選択した言語が維持される | |
