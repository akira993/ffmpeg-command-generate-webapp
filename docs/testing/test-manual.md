---
title: "手動テスト手順書"
description: "124項目の手動テストチェックリスト（UI・テーマ・プリセット・レスポンシブ・a11y）"
category: "testing"
created: "2026-02-17"
updated: "2026-03-11"
---
...
| 1-4 | コマンド出力エリアを確認 | 「生成されたコマンド」ヘッダー、「コピー」ボタンが表示 | |
| 1-5 | アクションボタンを確認 | **デスクトップのみ**: Separator下に「FFmpegの導入」「実行方法」「ライブラリの追加」の3ボタンが中央寄せで表示（モバイルでは `hidden`） | |
| 1-6a | フッターを確認 | 説明文と「FFmpegとは？」リンクが表示 | |
...
| # | 手順 | 期待結果 | OK/NG |
|---|------|---------|-------|
| 7.5-1 | 「ライブラリの追加」ボタンをクリック | ライブラリガイドモーダルが開く | |
| 7.5-2 | macOSタブを確認 | Homebrewでの説明と `brew reinstall ffmpeg` コマンドが表示 | |
| 7.5-3 | Windowsタブをクリック | winget/Chocolatey説明とgyan.devフルビルドへのリンクが表示 | |
| 7.5-4 | Linuxタブをクリック | Ubuntu/Debian, Fedora のコマンドが表示 | |
| 7.5-5 | ライブラリ確認方法 | `ffmpeg -encoders` コマンドが表示 | |
| 7.5-6 | ライブラリ一覧テーブル | 9行のコーデック/ライブラリ/用途テーブルが表示 | |
| 7.5-7 | モーダルスクロール | 内容がスクロール可能 | |
...
| # | 手順 | 期待結果 | OK/NG |
|---|------|---------|-------|
| 7.6-1 | 動画圧縮プリセットの動画コーデックラベル | 「動画コーデック」と表示（「コーデック」ではない） | |
| 7.6-2 | 音声コーデックラベル | 「音声コーデック」と表示 | |
| 7.6-3 | 動画コーデックドロップダウンを開く | 非デフォルトコーデック（H.265, VP8, VP9, AV1等）に「*」マーク付き | |
| 7.6-4 | 音声コーデックドロップダウンを開く | MP3, Opus, Vorbis に「*」マーク付き、AAC/FLAC/PCMは「*」なし | |
| 7.6-5 | 非デフォルト動画コーデック選択時 | amber色のヒント「* このコーデックは別途ライブラリのインストールが必要です」が表示 | |
| 7.6-6 | デフォルトコーデック（libx264）選択時 | ヒントが非表示 | |
...
| # | 手順 | 期待結果 | OK/NG |
|---|------|---------|-------|
| 7.7-1 | フッターの「FFmpegとは？」リンクをクリック | `/about-ffmpeg` ページに遷移 | |
| 7.7-2 | 日本語コンテンツ | 「FFmpegとは？」タイトル、概要、歴史、設計思想、ユースケース、コマンド例が表示 | |
| 7.7-3 | JPEG→AVIF比較表 | 10,000枚の比較テーブル（サイズ、削減率、SSIM、速度）が表示 | |
| 7.7-4 | H.264→AV1比較表 | 100本の比較テーブル（サイズ、削減率、VMAF、速度）が表示 | |
| 7.7-5 | 参考リンク | FFmpeg公式、GitHub、Wikipedia、Wiki、SVT-AV1 の5リンクが表示 | |
| 7.7-6 | CTAセクション | 「コマンドジェネレーターを使う」ボタンが表示、クリックで `/` に遷移 | |
| 7.7-7 | 戻るリンク | 「コマンドジェネレーターに戻る」リンクが表示、クリックで `/` に遷移 | |
| 7.7-8 | 英語切替 | ENに切替後、英語コンテンツが表示される | |
| 7.7-9 | メタタグ | `<title>` と `<meta description>` がlocaleに応じた内容 | |
...
| 8-4 | アクションボタンを確認 | "Install FFmpeg" / "How to Run" / "Add Libraries" に切り替わっている | |
...
| 9-3 | モバイル (375px) | プリセットが2列グリッド（v0.5.0で変更） | |
| 9-4 | モバイルでモーダル | モーダルが画面内に収まり、スクロール可能 | |
| 9-5 | モバイル固定バー | 画面下部固定バーに3ボタンが常時表示。**Separator下のActionButtonsは非表示**（重複なし） | |
| 9-6 | デスクトップでは固定バー非表示 | sm幅以上では画面下部固定バーが非表示 | |
| 9-7 | モバイルでコンテンツが固定バーに隠れない | 本体コンテンツに `pb-20`、フッターは CSS変数 `--mobile-bar-h`（ResizeObserver計測）で動的パディング。固定バーにコンテンツもフッターも隠れない | |
| 9-8 | iOS Safari dvh対応 | `min-h-dvh` によりブラウザツールバー表示時もフッターが画面内に収まる | |

### 9.9 コンパクトモード（PWA）

**前提:** PWAとしてインストール済みのスタンドアロンモードでテストする。

| # | 手順 | 期待結果 | OK/NG |
|---|------|---------|-------|
| 9-9-1 | PWAスタンドアロンでページ表示 | ヘッダー右側にコンパクトトグルボタン（Smartphoneアイコン）が表示 | |
| 9-9-2 | 通常ブラウザで表示 | コンパクトトグルボタンが非表示 | |
| 9-9-3 | トグルボタンをクリック | ウィンドウが1:3縦長アスペクト比にリサイズされる | |
| 9-9-4 | コンパクトモード中: ヘッダー | サブタイトルが非表示 | |
| 9-9-5 | コンパクトモード中: プリセットカード | 説明文（Card.Content）が非表示、アイコン+タイトルのみ表示 | |
| 9-9-6 | コンパクトモード中: セクション間余白 | `space-y-6` → `space-y-2` に圧縮されている | |
| 9-9-7 | コンパクトモード中: フッター | パディング圧縮（`py-1`）、動的底パディングは `--mobile-bar-h` で計算され固定バーと重ならない | |
| 9-9-7a | 固定バーテキスト折り返し時 | 画面幅を狭くしてテキストが折り返した場合も、フッターが固定バーに隠れない | |
| 9-9-8 | トグルボタンを再クリック | ウィンドウが元サイズに復元、全UI要素が通常表示に戻る | |
